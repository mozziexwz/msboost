...
MSBOOST v2 is a GPL-3.0-or-later derivative of FLVX `2.2.0-alpha4`, tailored
for game relay operations. The current v2 branch provides the forwarding,
node-agent, traffic accounting and monitoring foundation; MSBOOST account,
card-code, customer VPS deployment and configuration workflows are tracked in
[`plans/069-msboost-v2-migration.md`](plans/069-msboost-v2-migration.md).

## Test deployment

On a clean Debian 12 test VPS, run:

```bash
curl -fsSL https://raw.githubusercontent.com/mozziexwz/msboost/msboost-v2/msboost-install.sh | bash
```

The test panel listens on `6366` and the backend on `6365`. Do not expose it
to customers before changing the initial administrator password and placing it
behind HTTPS. The legacy Cloudflare Sites version remains on the `main` branch
until v2 acceptance testing is complete.

## Upstream notices

FLVX is copyright © 2026 Sagit-chu and is licensed under GPL-3.0-or-later.
It incorporates the Apache-2.0 licensed flux-panel project by bqlpfy. The
complete upstream notices are retained in [`NOTICE`](NOTICE),
